...
const Todos = () => {
  ...
  //// 查
  async function retrieveTodo(formData: FormData) {
    // 获取title
    const title = formData.get("title") as string
    // 清空输入框
    setMsg('')
    ...
    // 查找对应数据
    const data = await retrieveData(title)
    // 升序排列并渲染
    setTodos(data.sort((a, b) => a.id - b.id))
  }
  ...
  return (
    <fieldset className='flex flex-col items-center mx-auto w-1/3 mt-10 mb-10 pt-4 shadow-[0px_0px_60px_-15px_rgba(0,0,0,0.3)] rounded-lg divide-y'>
      <div className='flex self-stretch items-center mb-2'>
        ...
        <form className='ml-5 inline-flex grow items-center justify-around'>
          <input type="text" placeholder='write some todos ...'
            className="p-1 border pl-2 border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 placeholder:italic caret-sky-500 placeholder:text-slate-400 transition-all"
            name='title'
            value={msg}
      ...
      {!todos ? <p className='mb-2'>Waiting for fetching data...</p> : optimisticMsg.map(todo => (
        <div className='flex flex-row items-center last:mb-3 even:bg-slate-50' key={todo.id}>
          ...
          {todo.id === selectedId ?
            ...
            :
            <svg
              className='peer-has-[:checked]:invisible ml-2 p-0.5 mr-1 rounded cursor-pointer hover:bg-red-200 hover:stroke-red-400 active:scale-90 transition'
              width="18"
              height="18"
              viewBox="0 0 15 15"
              fill="none"
